...
exports.markAttendance = async (req, res) => {
    try {
        const { user_id, date, user_entry_time, user_exit_time, status } = req.body;
...
// Auto Random Attendance Generator
exports.autoGenerateAttendance = async (req, res) => {
    try {
        const { date } = req.body;
        ...
        const users = await User.find();
        const statuses = ["Present", "Absent", "HalfDay", "WorkFromHome"];
        ...
        const bulkOps = users.map(user => {
            ...
            if (status === "Present" || status === "WorkFromHome") {
                entryTime = "09:00 AM";
                exitTime = "06:00 PM";
            ...
            return {
                updateOne: {
                    filter: { user_id: user._id, date },
                    update: {
                        $set: {
                            user_entry_time: entryTime,
                            user_exit_time: exitTime,
                            status,
                            updated_at: new Date()
                        }
                    },
        ...
        console.error("Error in autoGenerateAttendance:", error);
        res.status(500).json({ message: "Server error" });
    }
};
...
exports.getMonthlyAttendance = async (req, res) => {
    try {
        ...
        const attendance = await Attendance.find({
            date: { $regex: `^${requestedYear}-${requestedMonth.toString().padStart(2, '0')}` }
        });
...
exports.calculateSalary = async (req, res) => {
    try {
        ...
        for (let record of attendanceRecords) {
            if (record.status === "HalfDay") {
                presentDays += 0.5;
            } else if (record.status === "Present" || record.status === "WorkFromHome") {
                presentDays += 1;
            }
        }
